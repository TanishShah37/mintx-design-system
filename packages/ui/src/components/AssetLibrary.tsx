...
const CATEGORY_COLORS: Record<IconCategory, { dot: string; label: string; bg: string; text: string }> = {
  navigation: { dot: "#10C9A0", label: "Navigation", bg: "rgba(0,179,138,0.1)", text: "#3DDCBA" },
  market: { dot: "#22C55E", label: "Market & Trading", bg: "rgba(34,197,94,0.1)", text: "#22C55E" },
  ...
  device: { dot: "#F87171", label: "Device & Platform", bg: "rgba(248,113,113,0.1)", text: "#F87171" },
...
const ILLUS_CATEGORY_META: Record<IllustrationCategory, { label: string; color: string; bg: string; musr: string }> = {
  "empty-state": { label: "Empty States", color: "#10C9A0", bg: "rgba(0,179,138,0.1)", musr: "MUSR-01 · MUSR-03 · MUSR-05 · MUSR-16 · MUSR-19" },
  ...
  error: { label: "Error States", color: "#F87171", bg: "rgba(248,113,113,0.1)", musr: "MUSR-02 · MUSR-09 · MUSR-18" },
  "market-status": { label: "Market Status", color: "#FBBF24", bg: "rgba(251,191,36,0.1)", musr: "MUSR-05 · MUSR-16" },
  success: { label: "Success & Celebration", color: "#22C55E", bg: "rgba(34,197,94,0.1)", musr: "MUSR-04 · MUSR-11 · MUSR-15 · MUSR-21" },
...
export function AssetLibrary({ 
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", color: "var(--text-primary, #E8EFED)", background: "var(--bg-base, #080E0D)", padding: "24px", borderRadius: "16px", border: "1px solid rgba(232,239,237,0.08)" }}>
      {showHeader && (
  ...
  return (
    <div>
      <div style={{ position: "relative", marginBottom: "20px" }}>
        <Search size={16} style={{ position: "absolute", left: "12px", top: "50%", transform: "translateY(-50%)", color: "#5A706A" }} />
        <input 
          value={query} 
...
function AssetCard({ name, id, type }: { name: string, id?: string, type: "icon" | "illustration" }) {
  ...
  return (
    ...
    >
      {copied && <span style={{ position: "absolute", top: "5px", right: "5px", background: "#22C55E", color: "#fff", fontSize: "10px", padding: "2px 6px", borderRadius: "4px" }}>Copied!</span>}
